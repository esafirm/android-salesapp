...
public class RxUi {
   ...
   * @param uiFunc not-null function that actually performs binding of the {@link Observable} to
   * something, for example UI.
  ...
  public static <T> Func1<Observable<T>, Subscription> ui(final Action1<T> uiAction) {
    return new Func1<Observable<T>, Subscription>() {
      @Override public Subscription call(Observable<T> observable) {
        return observable.observeOn(AndroidSchedulers.mainThread()).subscribe(uiAction);
      }
    };
  }
}